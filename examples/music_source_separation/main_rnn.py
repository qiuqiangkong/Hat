...
def train():
    ...
    lay_a1 = SimpleRnn( n_hid, act='tanh', name='a1' )( lay_in0 )    # Try LSTM, GRU instead
    ...
    lay_b1 = Dense( n_freq, act='sigmoid', name='a7' )( lay_a5 )     # mask_left, shape: (N, n_freq)
    lay_c1 = Dense( n_freq, act='sigmoid', name='a8' )( lay_a5 )     # mask_right, shape: (N, n_freq)
    ...
    md.fit( [np.abs(tr_y3d_chn0)+np.abs(tr_y3d_chn1)], [np.abs(tr_y2d_chn0), np.abs(tr_y2d_chn1)], \
        batch_size=100, n_epochs=100, loss_func=loss_func, optimizer=optimizer, callbacks=callbacks, verbose=1 )
